import { dbPool } from './dbPoolConfig.js';

export const calculateDailyTrending = async () => {
    const connection = await dbPool.getConnection();
    try {
        console.log("[DAILY TRENDING] Start calculating today's trending images...");

        const [rows] = await connection.query(`
      SELECT imageId, imageName, creator_irysId, imageStar AS starCount
      FROM images 
      ORDER BY imageStar DESC 
      LIMIT 50 
    `);

        if (rows.length === 0) {
            console.log("[DAILY TRENDING] No images found.");
            return;
        }

        const today = new Date().toISOString().split('T')[0];
        const insertData = rows.map((row, index) => [
            row.imageId,
            row.imageName,
            row.creator_irysId,
            row.starCount,
            'daily',
            today,
            index + 1,
            null,
            null,
            null
        ]);

        const insertQuery = `
      INSERT INTO trending_images (
        imageId, imageName, creator_irysId, starCount,
        periodType, periodDate, rankPosition,
        prevStarCount, increase, percentIncrease
      ) VALUES ?
    `;

        await connection.query(insertQuery, [insertData]);

        console.log(`[DAILY TRENDING] Successfully saved ${rows.length} today's trending images.`);
    } catch (error) {
        console.error("[DAILY TRENDING] Error:", error);
    } finally {
        connection.release();
    }
};